"""One separately armed, measured R201 carpet-tile calibration pulse."""
...
from r2_runtime.hil_preflight import validate_stock_bounded_calibration_preflight
...
SURFACE = "carpet_tile_8_in"
MAX_ACCEPTED_DISPLACEMENT_M = 0.25


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="One supervised stock R201 carpet-tile calibration"
    )
    parser.add_argument("--authorize-stock-bounded-calibration", action="store_true")
    for flag in (
        "operator-present",
        "device-inspected",
        "temperature-ok",
...
def main() -> None:
    ...
    try:
        preflight = validate_stock_bounded_calibration_preflight(args, os.environ)
    ...
    try:
        ...
        def pulse() -> None:
            owner.serialized(lambda: driver.dispatch_stock_heading_forward(SPEED))
        ...
        if result.terminal != "completed" or not result.stop_dispatched:
            raise RuntimeError("stock calibration watchdog did not complete with an emergency stop")
        time.sleep(0.25)
        owner.serialized(driver.dispatch_two_legs)
        time.sleep(0.25)
        owner.disconnect("stock_bounded_calibration_complete")
        ...
        try:
            ...
            pass
        try:
            owner.disconnect("stock_bounded_calibration_failed")
        except Exception:
            pass
    ...
    evidence: dict[str, object] = {
        "schema_version": "1.0",
        "evidence_category": "hil_stock_bounded_calibration",
        "generated_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "heading_processor": "default_primary",
        "surface": SURFACE,
        "speed": SPEED,
        ...
        "max_accepted_displacement_m": MAX_ACCEPTED_DISPLACEMENT_M,
        "measured_distance_m": None,
        "measurement_status": "operator_measurement_required",
    ...
    print(json.dumps({"report_sha256": digest, "requested_window_s": REQUESTED_WINDOW_S}))
