"""One separately armed stock-default R201 heading-control diagnostic pulse."""
...
from r2_runtime.hil_preflight import validate_stock_heading_diagnostic_preflight
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="One supervised stock R201 heading diagnostic")
    parser.add_argument("--authorize-stock-heading-diagnostic", action="store_true")
    for flag in (
        "operator-present",
        "device-inspected",
        "temperature-ok",
...
def main() -> None:
    ...
    try:
        preflight = validate_stock_heading_diagnostic_preflight(args, os.environ)
    ...
    try:
        ...
        if result.terminal != "completed" or not result.stop_dispatched:
            raise RuntimeError("stock-heading watchdog did not complete with an emergency stop")
        time.sleep(0.25)
        owner.serialized(driver.dispatch_two_legs)
        time.sleep(0.25)
        owner.disconnect("stock_heading_diagnostic_complete")
        ...
        try:
            ...
            pass
        try:
            owner.disconnect("stock_heading_diagnostic_failed")
        except Exception:
            pass
    ...
    evidence: dict[str, object] = {
        "schema_version": "1.0",
        "evidence_category": "hil_stock_heading_diagnostic",
