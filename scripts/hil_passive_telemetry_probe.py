"""One separately authorized, no-drive R201 passive telemetry sample."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="One no-drive R201 passive telemetry probe")
    ...
    backend = Spherov2LibraryBackend(
        policy=StationaryProbePolicy(
            ...
            allow_passive_telemetry_sample=True,
        )
    )
    ...
    try:
        ...
        detail = owner.serialized(lambda: backend.exercise_stationary("telemetry.passive_sample"))
        driver.safe_hold()
        owner.disconnect("passive_telemetry_probe_complete")
        ...
        try:
            ...
            pass
        try:
            owner.disconnect("passive_telemetry_probe_failed")
        except Exception:
            pass
        write_immutable_json(
            ...
            build_hil_failure_evidence(
                ...
                error_stage="passive_telemetry_sample",
            ),
        )
        raise SystemExit(2) from None
    evidence = {
        "schema_version": "1.0",
        "evidence_category": "hil_passive_telemetry_probe",
    ...
    print(json.dumps({"report_sha256": digest, "terminal": evidence["terminal"], **detail}))
