"""One separately armed, conservative R201 motion-calibration pulse.

The pulse is fixed at raw speed 5 for 0.10 seconds.  It is intentionally not a
general motor-control tool: the independent watchdog queues raw-motor OFF/0 on
normal completion, failure, or timeout and the evidence records only requested
bounds, never an inferred distance.
"""
...
from r2_runtime.hil_preflight import validate_bounded_motion_calibration_preflight
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="One supervised low-speed R201 calibration pulse")
    parser.add_argument("--authorize-bounded-motion-calibration", action="store_true")
...
def _success_evidence(terminal: str, elapsed_s: float) -> dict[str, object]:
    evidence: dict[str, object] = {
        "schema_version": "1.0",
        "evidence_category": "hil_bounded_motion_calibration_pulse",
...
def main() -> None:
    ...
    try:
        preflight = validate_bounded_motion_calibration_preflight(args, os.environ)
    ...
    try:
        ...
        if result.terminal != "completed" or not result.stop_dispatched:
            raise RuntimeError("bounded motion watchdog did not complete with an emergency stop")
        # Let the pinned sender write its final queued OFF/0.  This is not an acknowledgement.
        time.sleep(0.25)
        owner.disconnect("bounded_motion_calibration_complete")
        ...
        try:
            ...
            pass
        try:
            owner.disconnect("bounded_motion_calibration_failed")
        except Exception:
            pass
